...
put_stub = store_pb2_grpc.KeyValueStoreStub(channel)
stubs.append(put_stub)
stubs_available.append(f"{configuration['master']['ip']}:{configuration['master']['port']}")

for slave_config in configuration['slaves']:
    channel = grpc.insecure_channel(f"{slave_config['ip']}:{slave_config['port']}")
    stub = store_pb2_grpc.KeyValueStoreStub(channel)
    stubs.append(stub)
    stubs_available.append(f"{slave_config['ip']}:{slave_config['port']}")
    ...
    while True:
        ...
        if info:
            ...
            if re.match(r"^.+:.+$", info):
                ...
                response = put(stubs[0], key, value)
                break
